...
import { resolveStatus, resolveUserData } from './resolvers';
...
class AuthMech {
  ...
  connect (options) {
    if (this.options) {
      throw new Error('Cannot connect when already connected');
    };
  ...
  __setOptions (options) {
    if (!options.adapter) {
      throw new Error('No adapter was found in options');
    };
