...
class AuthMech {
  ...
  __linkService () {
    this.options.service.onAuthStateChanged(user => {
      this.state.userData = user ? cleanUser(user) : null;
  ...
  __patchFusion () {
    if (!this.state.status === STATUSES.SIGNEDIN) {
      this.options.docRef = null;
      this.__notifyStateChange();
      return;
    }

    const userId = this.state.userData.uid;
    const db = this.options.service.app.firestore();
    this.options.docRef = db
      .collection(this.options.fuse)
      .doc(userId);
    ...
    this.options.docRef
      ...
      .then(() => { // avoids double trigger if set concludes after the first snapshot
        this.options.docRef.onSnapshot(doc => {
          this.state.userData = {
            ...doc.data(),
            ...cleanUser(this.state.userData)
          };
          this.__notifyStateChange();
        });
      });
