...
class AuthMech {
  ...
  signUp (email, password) {
    ...
    const serviceUser = service.curentUser;
    return service
      .createUserWithEmailAndPassword(email, password)
      .then(() => serviceUser.sendEmailVerification());
  }
  ...
  updatePassword (newPassword, password) {
    ...
    return reauthenticate(service, email, password)
      ...
      .then(() => this.__reactToStateChange(EVENTS.PASSWORD_UPDATED, { ...this.state }));
  }
  ...
  updateFuse (props) {
    ...
    return this.config.docRef.update(props);
  }
};
